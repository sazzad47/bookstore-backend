...
echo PGPORT=$(aws ssm get-parameters --output text --region ap-south-1 --names PGPORT --with-decryption --query Parameters[4].Value | awk '{print int($0)}') >> /home/ec2-user/bookstore/.env
